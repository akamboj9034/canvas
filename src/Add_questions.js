...
export class Add_questions extends Component {
    ...
    componentDidUpdate() {
        ...
        if (this.state.authFlag == true) {
            ...
            let id = { quiz_id };
            axios.get(`http://localhost:3001/quiz_questions/${id.quiz_id}`, { headers: { 'Authorization': localStorage.getItem('token') } })
                .then((response) => {
    ...
    submitQuestion = (e) => {
        ...
        axios.post('http://localhost:3001/add_question', data, { headers: { 'Authorization': localStorage.getItem('token') } })
            .then(response => {
